...
<template>
    <div class="zdtb">
        ...
        <Row>
            <!-- 模态框 -->
            <Table :columns="$_columns7_$" :data="$_data6_$" style="clear:both;"></Table>
            <br/>
            <Col style="font-size: 14px; color: #000">共{{$_total_$}}条数据</Col>
        </Row>
        <br>
        <!-- page分页 -->
        <!-- <Row>
        ...
        <!-- 创建角色 -->
        <Modal v-model="$_modelRole_$" title="角色信息" :mask-closable="false" @on-cancel="$_quit_$('$_newRolev_$')">
        ...
        <!--编辑角色 -->
        <Modal v-model="$_rmModelEdit_$" title="角色信息" :mask-closable="false" @on-cancel="$_quit_$('$_bjRole1_$')">
            <Form ref="$_bjRole1_$" :model="$_bjRole_$" :rules="ruleValidate" :label-width="80">
                ...
                <FormItem label="角色层级" prop="type">
                    <Select v-model="$_bjtype_$" disabled placeholder="选择角色层级">
            ...
            <div slot="footer" align="center">
                <center>
                    <Button type="primary" size="large" @click="$_bjNew_$">保存</Button>
                </center>
                <!-- <Button size="large" @click="$_quit_$">取消</Button>
                <Button type="primary" size="large" @click="$_createNew_$" >保存</Button> -->
            </div>
        ...
        <Modal v-model="$_treeSetting_$" title="权限设置" :mask-closable="false">
            ...
            <div slot="footer">
                <center>
                    ...
                    <Button v-if="this.qx.flag==1" type="primary" size="large" @click="handleEditRights()">保存</Button>
                </center>
                <!-- <Button size="large" @click="editRightDialog()">取消</Button>
                        ...
                        this.$_sendQuery_$({
                            ...
                            if (rsp.status === 200) {
                                if (rsp.data.code === 0) {
                                    ...
                                    this.$_List_$();
                                    this.$_modelRole_$ = false;
                                    this.$refs.$_newRolev_$.resetFields();
                                } else {
                    ...
                    if (rsp.status === 200) {
                        if (rsp.data.code === 0) {
                            ...
                            if (role.flag == 1) {
                                ...
                                this.$_sendQuery_$({
                                    method: "GET",
                                    url: this.$_global_$.serverPath + `/user/resource/menu/all/` + 2,
                                    data: {},
                                    headers: {
                                        "Content-type": "application/json"
                                    }
                ...
                this.$_sendQuery_$({
                    ...
                    if (res.status === 200) {
                        if (res.data.code === 0) {
                            ...
                            this.$_treeSetting_$ = false;
                            this.$root.$_Route_$("company", "usermanage", "role", {});
                        }
                    }
                })
            },
