...
<template>
    <div>
        <div style="margin-left: 250px"><h2>停车场预约规则</h2></div>
        <br>
        <div title="停车场预约规则">
            <el-form ref="editForm" :model="$_addForm_$" :rules="addFormValidate" label-width="90px">
                <el-form-item label="规则描述：" prop="reserveRule">
                    <quill-editor ref="myTextEditor" v-model="$_addForm_$.reserveRule" :options="quillOption"></quill-editor>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 餐厅编辑保存
            $_saveEdit_$() {///zone/zone/{zoneId}/parking/rule
                this.$_sendQuery_$({
                    method: "POST",
                    url: `${this.$_global_$.serverPath}/zone/zone/${this.zoneId}/parking/rule`,
                    data: {                               
                        reserveRule: this.$_addForm_$.reserveRule,
                    },
                    ...
                    if (rsp.status === 200) {
                        if (rsp.data.code === 0) {
                            ...
                            this.$Message.error("修改停车场预约规则失败!")
                        }
                    }
                })
