...
<template>
    <div class="lm">
        <span style=" font-size: 14px; font-weight: bold">编辑企业</span>
        <br/>
        <div>
            <Form ref="$_formValidate_$" :model="$_formValidate_$" :rules="$_ruleValidate_$" :label-width="100">
                ...
                <FormItem label="注册资金：" prop="registeredCapital">
                    <Input style="width: 90%" v-model="$_formValidate_$.registeredCapital"
                           placeholder="请输入注册资金"></Input>&nbsp;&nbsp;&nbsp;万元
                </FormItem>
...
<script>
    ...
    export default {
        ...
        methods: {
            onBack() {
                this.$root.$_Route_$("superadmin", "system", "companymanage", {zoneId: this.zoneId});
            },
            //获取当前园区基本信息
            getZoneInfo() {
                this.$_sendQuery_$({
                    method: "GET",
                    url: `${this.$_global_$.serverPath}/zone/zone/${this.zoneId}`,
                }).then((rsp) => {
                    if (rsp.status === 200) {
                        if (rsp.data.code === 0) {
                ...
                if (this.$_formValidate_$.addressType == 0) {
                    this.$refs.$_formValidate_$.validate((valid) => {
                        if (valid) {
                            this.$_sendQuery_$({
                                method: "PUT",
                                url: `${this.$_global_$.serverPath}/zone/zone/${this.zoneId}/enterprise`,
                                data: {
